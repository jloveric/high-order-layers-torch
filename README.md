...
Simple XOR problem using the standard network struction (2 inputs 2 hidden 1 output) this will also work with not hidden layers. The function is discontinuous along the axis and we try and fit that function. Using piecewise discontinuous layers the model can match the function exactly.  
